...
const startCameraButton=document.getElementById("startCameraButton"),resultUrl=document.getElementById("resultUrl"),resultType=document.getElementById("resultType"),copyButton=document.getElementById("copyButton"),shareButton=document.getElementById("shareButton"),openButton=document.getElementById("openButton"),rescanButton=document.getElementById("rescanButton"),toast=document.getElementById("toast"),cameraSwitch=document.getElementById("cameraSwitch");
let stream=null,animationFrame=null,scanning=false,detected=false,currentValue="",facingMode="environment",lastDetectionTime=0,lastScanTime=0;
...
async function startCamera(){
  stopCamera();hideAllScreens();scannerScreen.classList.remove("hidden");statusElement.textContent="カメラを起動しています…";detected=false;lastDetectionTime=0;lastScanTime=0;
  try{
...
function scanFrame(now=performance.now()){
  if(!scanning)return;
...
async function copyCurrentValue(){if(!currentValue)return false;try{await navigator.clipboard.writeText(currentValue);return true}catch{const t=document.createElement("textarea");t.value=currentValue;t.style.cssText="position:fixed;opacity:0";document.body.appendChild(t);t.select();let ok=false;try{ok=document.execCommand("copy")}catch{}t.remove();return ok}}
